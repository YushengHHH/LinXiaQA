...
function buildComplaintProfile(topic:string){
 ...
 return {
  ...
  action:pickTag(topic,[["推进受阻",["推不动","推进","落地","执行","进度"]],["责任悬置",["负责","责任","没人","归口"]],["信息失真",["反馈","信息","沟通","说不清"]],["资源错配",["资源","人手","预算","时间","忙"]],["规则压制",["流程","审批","规则","制度"]]],"问题动作"),
  relation:pickTag(topic,[["上下传导",["高层","基层","一线","反馈","下达"]],["横向协作",["部门","协作","配合","对齐","扯皮"]],["目标-执行",["目标","执行","落地","交付"]],["规则-创新",["规则","流程","创新","试错"]]],"组织关系"),
  risk:pickTag(topic,[["停滞",["推不动","停","拖","慢"]],["内耗",["内耗","冲突","扯皮","争"]],["失焦",["模糊","混乱","散","不清楚"]],["失真",["反馈","信息","过滤","感知"]],["僵化",["规则","流程","审批"]]],"待观察风险")
 };
}
...
export default function Home(){
 ...
 const lens=getComplaintLens(topic);
 const profile=buildComplaintProfile(topic);
 ...
 function start(){setStep(-1);setAnswers([]);setPath(0);setSaved(false);nav("diagnose")}
 function submitComplaint(){setComplaint(topic);setStep(0)}
 function answer(value:AnswerValue){let next=[...answers];next[step]=value;setAnswers(next);if(step<2)setStep(step+1);else nav("map")}
 function save(){if(saved)return;let next=[{date:new Date().toLocaleDateString("zh-CN"),hex:hex.name+" · "+hex.title,target:chosen.target+" · "+chosen.name+" · "+topic,mode:modes[mode].name,progress:33},...records].slice(0,12);setRecords(next);setSaved(true);localStorage.setItem("linxia-fskn-records",JSON.stringify(next))}
 return <main>
