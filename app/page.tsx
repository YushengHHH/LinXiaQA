...
const changeScenes=[
 {from:"乾",to:"履",moving:2,fromLines:[1,1,1,1,1,1],toLines:[1,1,0,1,1,1]},
 {from:"履",to:"睽",moving:4,fromLines:[1,1,0,1,1,1],toLines:[1,1,0,1,0,1]},
 {from:"睽",to:"大有",moving:2,fromLines:[1,1,0,1,0,1],toLines:[1,1,1,1,0,1]},
 {from:"大有",to:"乾",moving:4,fromLines:[1,1,1,1,0,1],toLines:[1,1,1,1,1,1]}
];
function DynamicChange(){const[round,setRound]=useState(0);useEffect(()=>{const timer=setInterval(()=>setRound(v=>(v+1)%changeScenes.length),2600);return()=>clearInterval(timer)},[]);const scene=changeScenes[round];return <div className="change-stage" aria-label="动态变卦演示"><div className="change-meta"><span>动态变卦 · 一步一爻</span><b>{scene.from}之{scene.to}</b><small>只改变第 {scene.moving+1} 爻，卦名与象图同步更新</small></div><div className="six-lines">{scene.toLines.map((v,i)=><i className={`${v?"yang":"yin"} ${i===scene.moving?"moving":""}`} key={`${round}-${i}`}><span/><span/></i>)}</div><em className="change-word change">变易</em><em className="change-word constant">不易</em><em className="change-word simple">简易</em><div className="change-caption"><span>本卦 · {scene.from}</span><i>第 {scene.moving+1} 爻变</i><span>之卦 · {scene.to}</span></div></div>}
