...
function Sigil(){return <span className="sigil" aria-hidden="true"><i/><b/><em/></span>}
...
export default function Home(){
 ...
 return <main>
  <header className="top"><button className="brand" onClick={()=>nav("home")}><Sigil/><span><b>林下问路</b><small>知道管理 · WAYFINDING SYSTEM</small></span></button><nav><button onClick={start}>处境诊断</button><button onClick={()=>nav("system")}>方法体系</button><button onClick={()=>nav("history")}>演化记录 <i>{records.length}</i></button></nav></header>
  ...
  {screen==="home"&&<><section className="hero"><div className="hero-left"><p className="eyebrow">知道管理 × 林下问路</p><h1>先知其境，<br/><em>再问其路。</em></h1><p className="lead">一套面向复杂组织处境的诊断、趋势推演与自适应进化系统。<br/>它不替你预测吉凶，只帮助你看见：现在在哪里，不改变会去哪里，以及还能怎样走。</p><div className="actions"><button className="primary" onClick={start}>开始三问诊断 <span>↗</span></button><button className="link" onClick={()=>nav("system")}>先了解方法体系</button></div><div className="promise"><span>01 三问知境</span><span>02 三卦见势</span><span>03 三路寻优</span></div></div><div className="hero-orbit"><div className="orbit o1"/><div className="orbit o2"/><div className="orbit o3"/><span className="node n1">本卦<small>你在哪</small></span><span className="node n2">惯性之卦<small>不改变会怎样</small></span><span className="node n3">目标之卦<small>可以去哪里</small></span><div className="core"><Sigil/><b>知境</b><small>而后知道</small></div><p>每一次抵达，<br/>都是下一次重新寻优的起点。</p></div></section>
  <section className="triple"><div className="section-title"><p className="eyebrow">三卦路径沙盘</p><h2>不是一条静态路线，<br/>而是持续修正的航向。</h2></div>{[["本卦","极简自拍","把三次选择编码成当下处境"],["惯性之卦","默认趋势","推演非干预状态下的自然滑向"],["目标之卦","期望终点","比较代价，选择当前更好的归宿"]].map((v,i)=><article key={v[0]}><span>0{i+1}</span><h3>{v[0]}</h3><b>{v[1]}</b><p>{v[2]}</p><i/></article>)}</section>
  ...
  {screen==="system"&&<section className="system"><div className="system-hero"><p className="eyebrow">知道管理 · 方案总览</p><h1>以关系为体，<br/>以管理实践为用。</h1><p>这套方案把组织看作一个持续变化的关系网络。它的工作不是给复杂现实贴标签，而是把当下状态、默认趋势与可选目标放进同一个可计算、可讨论的框架。</p></div><div className="axioms">{[["公理 0","五行循环群","关系不是静态分类，而是可遍历的状态空间。"],["公理 1","三变完备性","次序 τ、构型 g、方向 δ，构成诊断入口。"],["公理 2","六爻投影","三变投影到组织六层，形成可观测结构。"]].map(v=><article key={v[0]}><small>{v[0]}</small><h2>{v[1]}</h2><p>{v[2]}</p></article>)}</div><div className="system-map"><aside><p className="eyebrow">产品闭环</p><h2>诊断不是终点，<br/>而是进入深度内容与持续问答的入口。</h2></aside><div>{[["诊断","我现在在哪","三问 → 八种处境"],["议题","为什么会这样","六十四种情境 · 七层阅读"],["问答","下一步怎么走","三锚点 · 200字行动指令"],["迭代","目标还对不对","内层修路 · 外层换靶"]].map((v,i)=><article key={v[0]}><span>0{i+1}</span><b>{v[0]}</b><h3>{v[1]}</h3><p>{v[2]}</p></article>)}</div></div><div className="principles"><p className="eyebrow">三重承诺</p><div><article><b>可解释</b><p>每一个判断都能回到三问、三卦与路径代价。</p></article><article><b>可选择</b><p>系统给出多条路线，目标与陪伴模式由用户决定。</p></article><article><b>可更新</b><p>变化后重新计算，旧答案不会永久定义组织。</p></article></div></div><button className="primary center" onClick={start}>从三问进入系统 <span>↗</span></button></section>}

  {screen==="history"&&<section className="history"><div className="section-title"><p className="eyebrow">演化记录 · 仅保存在此设备</p><h1>每次变卦，<br/>都是重新出发。</h1><p>这里记录的是判断如何更新，不是一个固定不变的组织画像。</p></div>{records.length?<div className="records">{records.map((r,i)=><article key={i}><time>{r.date}</time><div><small>本卦</small><b>{r.hex}</b></div><span>→</span><div><small>目标之卦</small><b>{r.target}</b></div><div className="record-progress"><i><em style={{width:`${r.progress}%`}}/></i><small>第 1 / 3 个中继节点 · {r.mode}</small></div></article>)}</div>:<div className="empty"><Sigil/><h2>尚未开始第一次演化</h2><p>完成三问并选择一条目标路径后，本次判断会留在这里。</p><button className="primary" onClick={start}>开始三问诊断</button></div>}</section>}
  <footer><div><Sigil/><b>林下问路</b></div><p>知境，而后知道。</p><span>知道管理 · 组织处境诊断、趋势推演与自适应进化系统</span></footer>
 </main>
}
