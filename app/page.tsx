...
function ReportSummary({hex,inertia,path,risk}:{hex:{name:string;title:string;symptom:string;inertiaTitle:string;warning:string};inertia:{name:string};path:{name:string;recommendation:string};risk:{label:string;tone:string}}){return <div className="report-summary"><small>诊断报告摘要 V0.1</small><ol><li><b>当前状态</b><span>你现在更接近「{hex.name} · {hex.title}」：{hex.symptom}</span></li><li><b>最大风险 · {risk.label}</b><span>若不干预，容易滑向「{inertia.name} · {hex.inertiaTitle}」：{hex.warning} {risk.tone}</span></li><li><b>建议先走</b><span>先走「{path.name}」：{path.recommendation}</span></li></ol></div>}
...
function rankStrategyPaths(paths:ReturnType<typeof buildStrategyPaths>,answers:(AnswerValue|undefined)[]){const scores=[6,4,2];answers.forEach(a=>{if(a?.bias!==undefined)scores[a.bias]+=(a.weight||1)});return paths.map((p,i)=>({...p,score:scores[i]})).sort((a,b)=>b.score-a.score)}
function WeightExplain({answers,score}:{answers:(AnswerValue|undefined)[];score:number}){const total=answers.reduce((sum,a)=>sum+(a?.weight||0),0),level=total>=8?"强":total>=5?"中":"轻";return <div className="weight-explain"><small>选项权重 V0.1</small><p>同一大类里的不同选项现在会产生不同强度。系统不会只看“选了哪一边”，也会看这个信号有多重，并据此调整路径排序。</p><span>本次信号强度：{level} · 总权重 {total||0}</span><span>当前首推路径权重：{score}</span></div>}
...
export default function Home(){
 ...
 const targetPaths=useMemo(()=>rankStrategyPaths(buildStrategyPaths(hex),answers),[hex,answers]);
