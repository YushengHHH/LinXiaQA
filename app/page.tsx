...
function DynamicChange(){const[lines,setLines]=useState([1,1,1,0,1,0]),[moving,setMoving]=useState(2),[round,setRound]=useState(0);useEffect(()=>{const timer=setInterval(()=>{setMoving(v=>{const next=(v+1)%6;setLines(old=>old.map((x,i)=>i===next?(x?0:1):x));return next});setRound(v=>(v+1)%4)},2600);return()=>clearInterval(timer)},[]);const names=["乾之履","履之同人","同人之大有","大有之乾"];return <div className="change-stage" aria-label="动态变卦演示"><div className="change-meta"><span>动态变卦 · 一步一爻</span><b>{names[round]}</b><small>每次只改变一个可验证的杠杆</small></div><div className="six-lines">{lines.map((v,i)=><i className={`${v?"yang":"yin"} ${i===moving?"moving":""}`} key={i}><span/><span/></i>)}</div><em className="change-word change">变易</em><em className="change-word constant">不易</em><em className="change-word simple">简易</em><div className="change-caption"><span>本卦</span><i>第 {moving+1} 爻正在变化</i><span>之卦</span></div></div>}
